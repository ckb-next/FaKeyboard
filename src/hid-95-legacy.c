...
const USB_DEVICE_DESCRIPTOR dev_dsc =
{
    ...
    0x1b1c,                 // Vendor ID
    0x1b06,                 // Product ID: Mouse in a circle fw demo
...
void handle_unknown_control(int sockfd, StandardDeviceRequest* control_req, USBIP_RET_SUBMIT* usb_req)
{
    ...
    if(control_req->bmRequestType == 192)
    {
        if(control_req->bRequest == 4)
        {
            /*char data[100] = {0};
            if ((recv (sockfd, data , control_req->wLength, 0)) != control_req->wLength)
            {
            ...
            usb_req->direction = 1;
            printf("Pre send\n");
            char buffer2[BSIZE] = { 0x02, 0x02, 0x03, 0x02, 0xff, 0x3c, 0x00, 0x00, 0x00, 0x06 };
            send_ctrl_response_response(sockfd, usb_req, buffer2, control_req->wLength, 0);
