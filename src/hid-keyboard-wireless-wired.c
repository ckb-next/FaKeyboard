...
const USB_DEVICE_DESCRIPTOR dev_dsc =
{
    ...
    0x1b1c,                 // Vendor ID
    0x1b45,                 // Product ID: Mouse in a circle fw demo
...
const CONFIG_HID  configuration_hid = {{
    ...
    // IF 2
    {
        ...
        0x01,         // Number of class descriptors, see usbcfg.h
        0x22,                // Report descriptor type
        0x008d,           // Size of the report descriptor
    }, {
        /* Endpoint Descriptor */
        0x07,/*sizeof(USB_EP_DSC)*/
        USB_DESCRIPTOR_ENDPOINT,    //Endpoint Descriptor
...
const byte corsair_report[141] =
{
    ...
    0x05, 0x01, 0x09, 0x80, 0xA1, 0x01, 0x85, 0x06, 0x19, 0x81, 0x29, 0x83, 0x15, 0x00, 0x25, 0x01, 
    0x95, 0x03, 0x75, 0x01, 0x81, 0x02, 0x95, 0x01, 0x75, 0x05, 0x81, 0x01, 0xC0,
};

const byte corsair_report2[25] =
{
...
void handle_data(int sockfd, USBIP_RET_SUBMIT* usb_req, int bl)
{
    if(usb_req->ep == 0x03)
    {
        if(usb_req->direction == 0) // Host Out
        {
            ...
            puts("");
            send_corsair_req(sockfd, usb_req, "", 0, 0);
            if((unsigned char)buffer[0] == 0x0e)
            {
                usb_req->seqnum = seqnum82;
                if(buffer[1] == 01)
                {
                    char response[BSIZE + 1] = { 0x0e, 0x01, 0x00, 0x00, 0x01, 0x01, 0x00, 0x01, 0x33, 0x00, 0x01, 0x03, 0x1c, 0x1b, 0x45, 0x1b, 0x01, 0x01, 0x00, 0x00, 0xc1, 0xff, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0xaa, 0x00, };
                    send_corsair_response(sockfd, usb_req, response, 64, 0, 0x82);
                }
                else if((unsigned char)buffer[1] == 0xae)
                {
                    char response[BSIZE + 1] = {   0x0e, 0xae, 0x00, 0x00, 0x1c, 0x1b, 0x45, 0x1b, 0x33, 0x00, 0x20, 0x52, 0x43, 0x01, 0x01, 0x00, };
                    send_corsair_response(sockfd, usb_req, response, 64, 0, 0x82);
                }
                else if((unsigned char)buffer[1] == 0x4a)
                {
                    char response[BSIZE + 1] = {   0x0e, 0x4a, 0x00, 0x00, 0x15, 0x8a, 0x1a, 0x95, 0x00, };
                    send_corsair_response(sockfd, usb_req, response, 64, 0, 0x82);
                }
                else if((unsigned char)buffer[1] == 0x51)
                {
                ...
                {
                    if((unsigned char)buffer[2] == 0x01)
                    {
                        char response[BSIZE + 1] = { 0x0e, 0x17, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2c, 0x01, 0x00, };
                        send_corsair_response(sockfd, usb_req, response, 64, 0, 0x82);
                    }
                    else
                    {
                        char response[BSIZE + 1] = {   0x0e, 0x17, 0x04, 0x00, 0x2d, 0x1b, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x2d, 0x1b, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x2d, 0x1b, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, };
                        send_corsair_response(sockfd, usb_req, response, 64, 0, 0x82);
                    }
                }
                else if((unsigned char)buffer[1] == 0x50)
                {
                    char response[BSIZE + 1] = { 0x0e, 0x50, 0x00, 0x00, 0x04, 0x02, 0x00, };
                    send_corsair_response(sockfd, usb_req, response, 64, 0, 0x82);
                }
                else if((unsigned char)buffer[1] == 0xa6)
                {
                    char response[BSIZE + 1] = { 0x0e, 0xa6, 0x00, 0x00, 0x03, 0x0f, 0x00, };
                    send_corsair_response(sockfd, usb_req, response, 64, 0, 0x82);
                }
                else
                {
        ...
        if(control_req->bRequest == 0x6)  //# Get Descriptor
        {
            if(control_req->wValue1 == 0x22)  // Send HID Reports
            {
                ...
                if(control_req->wIndex0 == 0x00)
                    send_usb_req(sockfd, usb_req, (char*) keyboard_report, 67, 0);
                else if(control_req->wIndex0 == 0x01)
                    send_usb_req(sockfd, usb_req, (char*) corsair_report, 141, 0);
                else if(control_req->wIndex0 == 0x02)
                    send_usb_req(sockfd, usb_req, (char*) corsair_report2, 25, 0);
                else
