...
const USB_DEVICE_DESCRIPTOR dev_dsc =
{
    ...
    0x1b1c,                 // Vendor ID
    0x1b35,                 // Product ID: Mouse in a circle fw demo
...
const CONFIG_HID_V3  configuration_hid = {{
        ...
        0x00ac,           // Size of the report descriptor
    }, {
        /* Endpoint Descriptor */
...
const byte keyboard_report[172] =
{
    ...
    0x15, 0x00, 0x26, 0xff, 0x00, 0x95, 0x3f, 0x75, 0x08, 0x09, 0x02, 0x81, 0x02, 0xc0, 0x06, 0xc2,
    0xff, 0x09, 0x03, 0xa1, 0x01, 0x85, 0x0e, 0x15, 0x00, 0x26, 0xff, 0x00, 0x95, 0x3f, 0x75, 0x08,
    0x09, 0x03, 0x81, 0x02, 0xc0, 0x05, 0x01, 0x09, 0x06, 0xa1, 0x01, 0x85, 0x05, 0x05, 0x07, 0x19,
    ...
    if(usb_req->ep == 0x02)
    {
        if(usb_req->direction == 0) // Host Out
        {
            ...
            if(buffer[0] == 0x0e)
            {
                ...
                if(buffer[1] == 01)
                {
                    char response[BSIZE + 1] = { 0x0e, 0x01, 0x00, 0x00, 0x01, 0x01, 0x00, 0x01, 0x33, 0x00, 0x01, 0x03, 0x1c, 0x1b, 0x35, 0x1b, 0x01, 0x01, 0x00, 0x00, 0xc1, 0xff, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0xaa, 0x00, };
                    send_corsair_response(sockfd, usb_req, response, 64, 0, 0x81);
                }
                else if((unsigned char)buffer[1] == 0xae)
                {
                    char response[BSIZE + 1] = {   0x0e, 0xae, 0x00, 0x00, 0x1c, 0x1b, 0x35, 0x1b, 0x33, 0x00, 0x20, 0x52, 0x43, 0x01, 0x01, 0x00, };
                    send_corsair_response(sockfd, usb_req, response, 64, 0, 0x81);
                }
        ...
        if(control_req->bRequest == 0x6)  //# Get Descriptor
        {
            if(control_req->wValue1 == 0x22)  // Send HID Reports
            {
                ...
                if(control_req->wIndex0 == 0x00)
                    send_usb_req(sockfd, usb_req, (char*) keyboard_report, 172, 0);
