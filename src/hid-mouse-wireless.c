    ...
    if(usb_req->ep == 0x02)
    {
        if(usb_req->direction == 0) // Host Out
        {
            ...
            if(buffer[0] == 0x0e)
            {
                ...
                if(buffer[1] == 01)
                {
                    char response[BSIZE + 1] = { 0x0e, 0x01, 0x00, 0x00, 0x01, 0x01, 0x00, 0x01, 0x33, 0x00, 0x01, 0x03, 0x1c, 0x1b, 0x64, 0x1b, 0x01, 0x01, 0x01, 0x05, 0xc0, 0xff, 0x40, 0x55, 0x00, };
                    send_corsair_response(sockfd, usb_req, response, 64, 0, 0x81);
                }
                else if((unsigned char)buffer[1] == 0xae)
                {
                    char response[BSIZE + 1] = {   0x0e, 0xae, 0x00, 0x00, 0x1c, 0x1b, 0x64, 0x1b, 0x33, 0x00, 0x20, 0x52, 0x43, 0x01, 0x01, 0x00, };
                    send_corsair_response(sockfd, usb_req, response, 64, 0, 0x81);
                }
