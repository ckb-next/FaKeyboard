...
const USB_DEVICE_DESCRIPTOR dev_dsc =
{
    ...
    0x048d,                 // Vendor ID
    0xc935,                 // Product ID: Mouse in a circle fw demo
...
const CONFIG_HID_MOUSEPAD  configuration_hid = {{
        ...
        0x00fc,           // Size of the report descriptor
    }, {
        /* Endpoint Descriptor */
...
const byte corsair_report[252] =
{
    ...
    0x09, 0x01, 0x15, 0x00, 0x26, 0xFF, 0x00, 0x75, 0x08, 0x95, 0xBF, 0xB1, 0x00, 0xC0, 0x06, 0x89,
    0xFF, 0x09, 0xCC, 0xA1, 0x01, 0x85, 0xCC, 0x09, 0x01, 0x15, 0x00, 0x26, 0xFF, 0x00, 0x75, 0x03,
    0x95, 0x20, 0xB1, 0x00, 0xC0, 0x05, 0x01, 0x09, 0x06, 0xA1, 0x01, 0x85, 0x01, 0x75, 0x01, 0x95,
    ...
    
};
    ...
    if(usb_req->ep == 0x01)
    {
        if(usb_req->direction == 0) // Host Out
        {
            ...
            if(buffer[0] == 0x0e)
            {
                ...
                if(buffer[1] == 01)
                {
                    char response[BSIZE + 1] = { 0x0e, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x02, 0x01, 0x03, 0x00, 0x1c, 0x1b, 0x3b, 0x1b, 0x08, 0x00, 0x00, 0x00, 0xc2, 0xff, 0x40, 0x00 };
        ...
        if(control_req->bRequest == 0x6)  //# Get Descriptor
        {
            if(control_req->wValue1 == 0x22)  // Send HID Reports
            {
                ...
                if(control_req->wIndex0 == 0x00)
                    send_usb_req(sockfd, usb_req, (char*) corsair_report, 252, 0);
    ...
    if(control_req->bmRequestType == 0x21)  // Host Request
    {
        ...
        if(control_req->bRequest == 0x09)  // set report
        {
           ...
           /* if(control_req->wLength > 20)
            ...
            char buffer2[2000] = {0xFF};
            send_ctrl_response(sockfd, usb_req, buffer2, control_req->wLength, 0);
        }
    }
    if(control_req->bmRequestType == 0xa1 && control_req->bRequest == 1)
    {
        char buffer2[5] = {0x07, 0x01, 0x01, 0x00, 0x6e};
        //char buffer2[5] = {0x07, 0x01, 0x01, 0x00, 0x64};
        if(hack)
        {
            buffer2[1] = 0x04;
                FILE* f = fopen("byte.txt", "rb");
                fscanf(f, "0x%hhx", buffer2 + 4);
                fclose(f);
                hack--;
        } else {
            hack++;
        }
        send_ctrl_response_response(sockfd, usb_req, buffer2, 5, 0);
        printf("\n\nSENDING HACK\n\n");
    }
};
